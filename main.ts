    ...
    await Promise.all(characterList.map(async character => {
        if (options.selectAttendanceChannel === '0' || options.selectAttendanceChannel === character.channelMasterId) {
            ...
            if (hideName) {
                ...
                console.log(`将签到 ${character.nickName}`);
            }
            if (data.code === 0 && data.message === 'OK') {
                const msg = `${hideName ? '' : (Number(character.channelMasterId) - 1) ? 'B 服' : '官服'}角色 ${hideName ? successAttendence + 1 : character.nickName} 签到成功${', 获得了' + data.data.awards.map(a => '「' + a.resource.name + '」' + a.count + '个').join(',')}`;
                combineMessage(msg);
                successAttendence++;
            } else {
                const msg = `${hideName ? '' : (Number(character.channelMasterId) - 1) ? 'B 服' : '官服'}角色 ${hideName ? successAttendence + 1 : character.nickName} 签到失败${hideName ? '' : `, 错误消息: ${data.message}\n\n\`\`\`json\n${JSON.stringify(data, null, 2)}\n\`\`\``}`;
...
const hideName = process.env.HIDE_NAME ? Boolean(Number(process.env.HIDE_NAME)) : false

await Promise.all(accounts.map(token => doAttendanceForAccount(token, { withServerChan, withBark, selectAttendanceChannel, hideName })))